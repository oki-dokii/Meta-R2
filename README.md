...
│   │     LifeStackAgent      │                                       │
│   │  ┌───────────────────┐  │                                       │
│   │  │  Qwen2.5-1.5B     │  │  ← GRPO-fine-tuned LoRA adapter      │
│   │  │  + LoRA adapter   │  │    (jdsb06/lifestack-grpo-v4)        │
│   │  └───────────────────┘  │                                       │
│   │  ┌───────────────────┐  │                                       │
│   │  │  LifeStackMemory  │  │  ← ChromaDB · 3 collections          │
│   │  │  (ChromaDB)       │  │    decisions / trajectories / feedback│
│   │  └───────────────────┘  │                                       │
│   └────────────┬────────────┘                                       │
...
│   │                                                             │  │
│   │  ┌──────────────┐   ┌─────────────────┐  ┌─────────────┐  │  │
│   │  │  WorldEngine │   │ DependencyGraph  │  │LifeStackVer-│  │  │
│   │  │  (ExoEvents) │──►│  32 edges        │  │ifier        │  │  │
│   │  └──────────────┘   │  0.6 dampening   │  │(anti-hack)  │  │  │
│   │                     └────────┬─────────┘  └─────────────┘  │  │
│   │                              │                              │  │
...
│   │                              │                              │  │
│   │                    10 reward functions                      │  │
│   │              + 7-day γ=0.9 discounted rollout               │  │
│   └─────────────────────────────────────────────────────────────┘  │
└─────────────────────────────────────────────────────────────────────┘

Training loop (offline, scripts/train_trl.py):
  Qwen2.5-1.5B-Instruct → LifeStackGRPOTrainer → 5-stage curriculum
  → JSON-boundary masked gradients → LoRA checkpoint → HF Hub
```
